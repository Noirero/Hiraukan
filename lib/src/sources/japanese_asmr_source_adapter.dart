...
class JapaneseAsmrLastKnownGood {
  ...
  static List<SourceWorkCandidate> candidates() {
    return entries.map((entry) {
      final voices = entry.voice
          .split(RegExp(r'[,、/]'))
          ...
          .toList(growable: false);
      final circle = entry.circle.trim().isEmpty ? null : entry.circle.trim();
      final ref = UnifiedSourceRef(
        source: UnifiedSourceKind.japaneseAsmr,
        localId: entry.rj,
        canonicalId: entry.rj,
      ...
      final work = Work(
        id: SourceHtmlParser.stableNegativeId('japaneseasmr:${entry.rj}'),
        title: entry.title,
        name: circle,
        age: 'R18',
        release: _releaseFromCode(entry.releaseCode),
        vas: voices.isEmpty ? null : voices,
        images: <String>[entry.coverUrl],
  ...
  static String? _releaseFromCode(String value) {
    if (value.length != 6) return null;
    final yearPart = int.tryParse(value.substring(0, 2));
    final month = int.tryParse(value.substring(2, 4));
    final day = int.tryParse(value.substring(4, 6));
    if (yearPart == null || month == null || day == null) return null;
...
class JapaneseAsmrSourceAdapter extends HtmlAudioSiteSourceAdapter {
  ...
  SourceSearchPage _searchLastKnownGood({
    ...
    var items = JapaneseAsmrLastKnownGood.candidates();
